    ...
    return new Promise((resolve, reject) => {
      ...
      const handleError = (host, error) => {
        ...
        if (pendingRequests === 0 && !hasResolved) {
          ...
          if (elapsedTime < this.maxAttemptTime && isAttempting) {
            // Reset pending requests and try again with new random hosts
            pendingRequests = hosts.length;
            ...
            newHosts.forEach(newHost => {
              this.makeRequest(newHost, endpoint, 'POST', requestData, apiKey, isStream)
                ...
                .catch(error => {
                  handleError(newHost, error);
                });
            });
          } else if (!hasResolved) {
            cleanup();
            reject(new Error('All hosts failed to respond after maximum attempt time'));
          }
        }
      };
